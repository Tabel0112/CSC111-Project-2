...
def build_default_shock_lookup() -> dict[str, float]:
    ...
    return dict(zip(default_names, shocks))
...
def sync_shock_rows(
    ...
    existing_lookup = {
        str(row.get("code", "")): safe_float(
            row.get("shock", DEFAULT_INITIAL_SHOCK),
            default=DEFAULT_INITIAL_SHOCK,
        )
    ...
    default_code_lookup = {
        country_code: default_lookup.get(country.name, DEFAULT_INITIAL_SHOCK)
        for country_code, country in countries.items()
    }

    rows = []
    for selected_code in selected_codes:
        if selected_code not in countries:
            continue
        rows.append(
            {
                "code": selected_code,
                "country": countries[selected_code].name,
                "shock": existing_lookup.get(
                    selected_code,
                    default_code_lookup.get(selected_code, DEFAULT_INITIAL_SHOCK),
                ),
            }
        )
    return rows
...
def _find_available_port(start_port: int, host: str = "127.0.0.1", tries: int = 20) -> int:
    ...
    raise OSError(
        f"Could not find an available port between {start_port} and {start_port + tries - 1}."
    )
...
def _run_simulation_from_controls(
    ...
    initial_shock_map = {
        str(row["code"]): safe_float(row["shock"], default=DEFAULT_INITIAL_SHOCK)
        for row in rows
...
def _build_slider_marks(step_count: int) -> dict[int, str]:
    ...
    if step_count <= 8:
        ...
        for step_index in range(step_count):
            marks[step_index] = str(step_index)
        return marks

    stride = max(1, step_count // 6)
    marks = {0: "0", step_count - 1: str(step_count - 1)}
    for step_index in range(stride, step_count - 1, stride):
        marks[step_index] = str(step_index)
...
def _render_dashboard_figure(
    ...
    if not simulation_data:
        return create_simulation_figure(
            base_countries,
            [
                {
                    "step": 0,
            ...
            set(list(base_countries)[: min(20, len(base_countries))]),
            show_edges=False,
...
def run_dashboard(args: Namespace) -> None:
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "Dash is not installed. Run `pip install -r requirements.txt` first."
        ) from exc
    ...
    app.layout = html.Div(
        ...
        children=[
            html.H1("Macroeconomic Shock Simulator"),
    ...
    def _run_from_controls(
        ...
        top_k: int,
        hide_edges: list[str],
    ) -> tuple[dict[str, object], str, int, int, dict[int, str], dict[str, bool], bool, str]:
        ...
        step_count = len(step_history) if isinstance(step_history, list) else 0
        return (
            simulation_data,
            status,
            max(0, step_count - 1),
    ...
    def _control_playback(
        ...
        return (
            current_value,
            playback_state,
            (not playback_state.get("playing", False)),
            ("Pause" if playback_state.get("playing", False) else "Play"),
        )
