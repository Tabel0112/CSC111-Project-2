...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no-prompt",
        action="store_true",
        help="Skip interactive terminal prompts when using the CLI fallback.",
    )
    parser.add_argument(
        "--cli",
        action="store_true",
        help="Use the older terminal + Plotly flow instead of the default Dash UI.",
    )
...
def _print_run_summary(
    ...
    print("Mode: real time-step model")
    print(f"Initial shock countries: {', '.join(country_names)}")
    print(f"Generated {len(wave_history)} steps.")
...
def main() -> None:
    ...
    display_countries = clone_trade_graph(countries)
    limit_top_k_partners(display_countries, args.top_k)
    ...
    wave_history = run_time_step_simulation(
        countries,
        initial_shocks,
